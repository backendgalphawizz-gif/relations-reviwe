...
define('DESTINATIONPATH', 'storage/images/');
...
class UserController extends Controller
{
        ...
        try {
            ...
            if ($req->profile) {
                if (Str::contains($req->profile, 'storage')) {
                    ...
                    File::delete($path);
                    file_put_contents(public_path($path), base64_decode($req->profile));
                }
            } else {
                $path = null;
            }
    ...
    public function get_pages($slug)
    {
        try {
            ...
            $user = DB::table('static_pages')->where('slug',$slug)->first();
    ...
    public function updateUser(Request $req, $id)
    {
        try {
            ...
            $validator = Validator::make($req->all(), [
                'contactNo' => 'required|max:100|unique:users,contactNo,'.$id,
            ]);
    ...
    public function sendAdvisorEnquiry(Request $req)
    {
        try {
            $validator = Validator::make($req->all(), [
            ...
            $userId = Auth::guard('api')->user()->id ?? $req->user_id ?? null;

            $id = DB::table('astrologer_enquiries')->insertGetId([
                'user_id' => $userId,
                'name' => $req->name,
                'email' => $req->email,
                'mobile' => $req->mobile,
    ...
    public function sendAdsEnquiry(Request $req)
    {
        try {
            $validator = Validator::make($req->all(), [
            ...
            $startDateInput = $req->input('start_date')
                ?? $req->input('startDate')
                ?? $req->input('start-date');
            ...
            $startDate = $this->parseFlexibleDate($startDateInput);
            if (!$startDate) {
                return response()->json([
                    'error' => [
            ...
            $id = DB::table('ad_enquiries')->insertGetId([
                'user_id' => $userId,
    ...
    private function parseFlexibleDate($value)
    {
        ...
        foreach ($formats as $format) {
            try {
                ...
                if ($parsed && $parsed->format($format) === $value || $parsed) {
        ...
        try {
            ...
            // continue to strtotime fallback
        }
        ...
        if (preg_match('/(\d{1,2})[\/\-\.](\d{1,2})[\/\-\.](\d{2,4})/', $value, $matches)) {
            ...
            if ($part1 > 12) {
                return sprintf('%04d-%02d-%02d', $part3, $part2, $part1);
            }
            if ($part2 > 12) {
                return sprintf('%04d-%02d-%02d', $part3, $part1, $part2);
            }
